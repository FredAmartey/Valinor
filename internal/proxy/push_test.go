...
func TestPushConfig_Success(t *testing.T) {
	...
	err = proxy.PushConfig(ctx, pool, "agent-1", cid, config, allowlist, nil, nil, nil, 5*time.Second)
	require.NoError(t, err)
...
func TestPushConfig_NoListener(t *testing.T) {
	...
	err := proxy.PushConfig(context.Background(), pool, "agent-bad", 99, nil, nil, nil, nil, nil, 2*time.Second)
	assert.Error(t, err)
}
...
func TestPushConfig_Timeout(t *testing.T) {
	...
	err = proxy.PushConfig(ctx, pool, "agent-1", cid, nil, nil, nil, nil, nil, 500*time.Millisecond)
	assert.Error(t, err)
}
