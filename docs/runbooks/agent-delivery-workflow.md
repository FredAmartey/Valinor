...
- Ship in vertical slices with explicit acceptance criteria.
...
- Be sized appropriately for 1 focused PR when possible.
...
- Trigger manual review on the PR immediately (command depends on your reviewer bot/tool). Run review in parallel with CI/security checks — do not wait for checks to pass first.
- If findings appear:
